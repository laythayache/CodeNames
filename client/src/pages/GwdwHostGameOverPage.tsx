...
import type { GwdwPlayerScore, GwdwAward } from "shared/types";
...
export function GwdwHostGameOverPage() {
  const { state } = useGame();
  ...
  const hostDisplay = state.gwdwHostDisplay;
  const winnerPlayer = hostDisplay?.players.find((p) => p.displayName === winner);

  const handleNewGame = () => {
    getSocket().emit("client:gwdw-rematch");
  };
  ...
  return (
    <div className="min-h-dvh bg-gray-950 text-white overflow-hidden relative">
      ...
      <div className="absolute inset-0 bg-linear-to-b from-teal-950/30 via-gray-950 to-gray-950 pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-dvh px-6 py-8">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-gray-500">
                        {s.correctGuesses} correct &middot; evaded {s.timesEvaded} &middot; identified {s.timesIdentified}
                      </div>
                    </div>
                  </div>
                  <span className="text-2xl font-bold text-teal-400">{s.score}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {showNewGame && (
          <button
            ...
            className="px-10 py-5 bg-teal-600 text-white rounded-2xl font-bold text-xl sm:text-2xl
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-2">
        ...
        <div className={`font-bold ${place === 1 ? "text-2xl text-yellow-400" : "text-lg text-teal-400"}`}>
          {player.score}
        </div>
      </div>
...
function AwardCard({ award }: { award: GwdwAward }) {
  return (
    <div className="bg-gray-900/80 border border-gray-700 rounded-xl p-4 text-center
      ...
      <div className="text-lg font-bold text-teal-300">{award.playerName}</div>
...
function ConfettiEffect() {
  const pieces = Array.from({ length: 40 }, (_, i) => ({
    ...
    color: ["#14b8a6", "#facc15", "#f97316", "#22c55e", "#3b82f6", "#ec4899"][
